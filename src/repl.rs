...
pub fn repl() -> std::result::Result<(), HyperkitError> {
        ...
        match e {
            ReadlineError::Io(e) => match e.kind() {
                std::io::ErrorKind::NotFound => eprintln!(
                    ...
                    "History file doesn't exist".bright_red().bold()
                ),
                _ => {}
            },
            _ => {}
    ...
    loop {
        ...
        match tok1.trim() {
            ...
            "mk" => {
                let tok2 = token(&data, 1)
                    .checker(Some("directory name".to_string()))
                    .ughf();
            ...
            "rn" => {
                ...
                let tok2 = token(&data, 2)
                    .checker(Some("output path or name".to_string()))
                    .ughf();
            ...
            "run" => {
                let tok1 = token(&data, 1)
                    .checker(Some("command name".to_string()))
                    .ughf();
            ...
            "calc" => {
                let tok1 = token(&data, 1)
                    .checker(Some("expression".to_string()))
                    .ughf();
            ...
            "tar" => {
                ...
                let fname = token(&data, 2)
                    .checker(Some("path or file".to_string()))
                    .ughf();
            ...
            "transmute" => {
                ...
                let fname = token(&data, 3)
                    .checker(Some("file name".to_string()))
                    .ughf();
                let outname = token(&data, 4)
                    .checker(Some("output name".to_string()))
                    .ughf();
            ...
            "history" => {
                ...
                if let Ok(o) = flag {
                    if o == "--clean" {
                        def.clear_history()
                            .errh(Some("history".to_string()))
                            ._success_res("history", "Cleaned")
                            .ughv();
                    } else {
            ...
            _ => {
                if !tok1.is_empty() {
                    println!(
                        "[{path:?}]~>{}: [\x1b[36m{}\x1b[0m] please try again",
